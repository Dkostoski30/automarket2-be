...
/**
 * Boots payment-service against a real PostgreSQL.
 *
...
class PaymentSchemaMigrationTest {
    ...
    void migratesIntoItsOwnFlywayHistoryTable() {
        Integer applied = jdbc.queryForObject(
                "SELECT count(*) FROM flyway_history_payment WHERE success", Integer.class);

        assertThat(applied)
                .as("payment-service must record its migrations in flyway_history_payment, not the shared default")
                .isNotNull()
                .isPositive();
    }
