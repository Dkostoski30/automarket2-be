...
import com.automarket.payment.repository.SubscriptionRepository;
import com.automarket.payment.repository.UserViewRepository;
...
public class SubscriptionService {
    ...
    public String createCheckoutSession(String userEmail, String plan, String successUrl, String cancelUrl) {
        ...
        UserView user = userViewRepository.findByEmail(userEmail)
    ...
    private String resolveOrCreateStripeCustomer(UserView user) throws StripeException {
        return subscriptionRepository.findByUserId(user.getId())
    ...
    private void handleCheckoutCompleted(Session session) {
        ...
        Subscription sub = subscriptionRepository.findByUserId(userUuid)
                .orElseGet(() -> Subscription.builder().userId(userUuid).build());
        sub.setPlan(Subscription.Plan.valueOf(plan.toUpperCase()));
        ...
        // Publish event instead of directly updating User.plan
        eventPublisher.publishSubscriptionActivated(userUuid, plan.toUpperCase(), session.getSubscription());
    }
    ...
    private void handleSubscriptionDeleted(com.stripe.model.Subscription stripeSubscription) {
        subscriptionRepository.findByStripeSubscriptionId(stripeSubscription.getId()).ifPresent(sub -> {
            ...
            log.info("Subscription cancelled: id={} userId={}", stripeSubscription.getId(), sub.getUserId());

            // Publish event instead of directly updating User.plan
            eventPublisher.publishSubscriptionCancelled(sub.getUserId(), sub.getPlan().name(), stripeSubscription.getId());
        });
    }
