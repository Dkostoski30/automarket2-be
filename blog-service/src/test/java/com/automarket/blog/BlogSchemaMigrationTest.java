...
/**
 * Boots blog-service against a real PostgreSQL.
 *
...
class BlogSchemaMigrationTest {
    ...
    void migratesIntoItsOwnFlywayHistoryTable() {
        Integer applied = jdbc.queryForObject(
                "SELECT count(*) FROM flyway_history_blog WHERE success", Integer.class);

        assertThat(applied)
                .as("blog-service must record its migrations in flyway_history_blog, not the shared default")
                .isNotNull()
                .isPositive();
    }
