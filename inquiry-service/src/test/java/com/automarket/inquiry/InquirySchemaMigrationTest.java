...
/**
 * Boots inquiry-service against a real PostgreSQL.
 *
...
class InquirySchemaMigrationTest {
    ...
    void migratesIntoItsOwnFlywayHistoryTable() {
        Integer applied = jdbc.queryForObject(
                "SELECT count(*) FROM flyway_history_inquiry WHERE success", Integer.class);

        assertThat(applied)
                .as("inquiry-service must record its migrations in flyway_history_inquiry, not the shared default")
                .isNotNull()
                .isPositive();
    }
