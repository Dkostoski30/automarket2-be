package com.automarket.listing;
...
/**
 * Boots listing-service against a real PostgreSQL.
 *
...
class ListingSchemaMigrationTest {
    ...
    void migratesIntoItsOwnFlywayHistoryTable() {
        Integer applied = jdbc.queryForObject(
                "SELECT count(*) FROM flyway_history_listing WHERE success", Integer.class);

        assertThat(applied)
                .as("listing-service must record its migrations in flyway_history_listing, not the shared default")
                .isNotNull()
                .isPositive();
    }
