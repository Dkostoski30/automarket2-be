...
import com.automarket.listing.config.CacheConfig;
import com.automarket.listing.dto.*;
import com.automarket.listing.entity.*;
import com.automarket.listing.repository.*;
import com.automarket.listing.specification.ListingSpecification;
...
public class ListingService {
    ...
    public PageResponse<ListingDto> getMyListings(String email, int page, int size) {
        UserView user = getUserByEmail(email);
        Page<Listing> results = listingRepository.findBySellerId(
    ...
    public ListingDetailDto create(CreateListingRequest request, String sellerEmail) {
        UserView seller = getUserByEmail(sellerEmail);
    ...
    private void enforceListingLimit(UserView seller) {
        if (seller.getPlan() == UserView.Plan.FREE) {
            long activeCount = listingRepository.countActiveBySellerId(seller.getId());
    ...
    private Listing getListingAndCheckOwnership(UUID listingId, String requesterEmail) {
        ...
        UserView requester = getUserByEmail(requesterEmail);
    ...
    private UserView getUserByEmail(String email) {
